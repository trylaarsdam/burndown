...
const selectProject = () => {
	if (selected_project.value != "") {
		store.project_id = selected_project.value;
		router.push("/stage/choose_milestone")
	}
	else {
			...
			<div class="mx-auto max-w-md">
				<div class="content-left">
					<h6>
						...
						<span class="text-gray-900 text-lg font-semibold pl-4">Choose a Project</span>
					</h6>
				...
				<div class="divide-y divide-gray-300/50">
					<div class="text-base font-semibold leading-7 flex" v-if="loading">
						...
						<span class="text-gray-900">Loading Projects...</span>
					</div>
					<div v-if="!loading && !error">
						<div class="text-base font-semibold leading-7 flex" >
							<label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select an option</label>
								<select v-model="selected_project" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
									<option disabled value="">Choose a project</option>
									<option v-for="option in projects" :value="option.id.toString()" :key="option.id.toString()">{{option.name_with_namespace}}</option>
								</select>
						</div>
						<button :disabled="selected_project == ''" @click="selectProject" class="mt-4 bg-green-500 disabled:bg-gray-500 text-white font-semibold py-2 px-4 rounded">Continue</button>
					</div>
					<div class="text-base leading-7" v-if="error">
						<span class="text-red-500 font-semibold">An error occurred while fetching your projects.</span>
